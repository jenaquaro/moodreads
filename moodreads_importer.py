...
def get_database_properties(notion_token: str, database_id: str) -> set[str]:
    """Fetch the list of property names from a Notion database.
    ...
    Returns:
        Set of property names that exist in the database.
    """
    ...
    try:
        ...
        logger.warning("Failed to fetch database schema: %s", e)
        return set()


# Global cache for database properties
_db_properties: set[str] | None = None


def get_cached_db_properties(notion_token: str, database_id: str) -> set[str]:
    """Get database properties with caching."""
    global _db_properties
    if _db_properties is None:
        _db_properties = get_database_properties(notion_token, database_id)
        logger.info("Database has %d properties", len(_db_properties))
    return _db_properties


def build_notion_properties(
    row: dict[str, str], meta: dict[str, Any], valid_properties: set[str] | None = None
) -> dict[str, Any]:
    ...
    Args:
        ...
        valid_properties: Set of property names that exist in the database.
            If provided, only these properties will be included.
    ...
    def should_include(prop_name: str) -> bool:
        """Check if property should be included based on valid_properties."""
        if valid_properties is None:
            return True
        return prop_name in valid_properties
...
def main() -> int:
    ...
    db_properties = get_cached_db_properties(notion_token, database_id)
    if not db_properties:
        logger.error("Could not fetch database properties. Check your NOTION_TOKEN and NOTION_DATABASE_ID.")
        return 1
    ...
    for idx, row in enumerate(rows, start=1):
        ...
        # Build Notion properties (only include properties that exist in the database)
        properties = build_notion_properties(row, meta, valid_properties=db_properties)
